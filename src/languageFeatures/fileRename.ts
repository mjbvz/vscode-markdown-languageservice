...
import { isExcludedPath, LsConfiguration, PreferredMdPathExtensionStyleChange } from '../config';
...
export class MdFileRenameProvider extends Disposable {
	...
	#shouldRemoveFileExtensionForRename(originalHref: InternalHref, newUri: URI): boolean {
		...
		if (this.#config.preferredMdPathExtensionStyle === PreferredMdPathExtensionStyleChange.removeExtension) {
			return true;
		}
