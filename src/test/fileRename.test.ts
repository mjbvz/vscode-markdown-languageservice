...
suite('File Rename', () => {
	...
	test('Rename file should drop file extensions if it has been configured to', withStore(async (store) => {
		...
		const response = await getFileRenameEdits(store, [{ oldUri, newUri }], workspace, { preferredMdPathExtensionStyle: PreferredMdPathExtensionStyleChange.removeExtension });
		assertEditsEqual(response!.edit, {
			uri: docUri, edits: [
